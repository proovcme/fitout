...
export class NavigationGrid{
  constructor({minX=-8,maxX=8,minZ=-6.5,maxZ=6.5,cellSize=.5,blocked=[]}={}){this.minX=minX;this.maxX=maxX;this.minZ=minZ;this.maxZ=maxZ;this.cellSize=cellSize;this.cols=Math.floor((maxX-minX)/cellSize)+1;this.rows=Math.floor((maxZ-minZ)/cellSize)+1;this.blocked=new Set();this.setBlockedRects(blocked)}
  ...
  setBlockedRects(rects=[]){this.blocked.clear();for(const rect of rects)this.blockRect(rect)}
