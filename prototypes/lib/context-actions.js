export class ContextActions{
  ...
  update(){let best=null,bestDistance=Infinity,bestPriority=-Infinity;for(const entry of this.entries){if(!entry.enabled()||entry.object.visible===false)continue;const distance=this.actor.position.distanceTo(entry.object.position);if(distance<=entry.radius&&(entry.priority>bestPriority||entry.priority===bestPriority&&distance<bestDistance)){best=entry;bestDistance=distance;bestPriority=entry.priority}}this.active=best;this.promptElement.classList.toggle('show',Boolean(best));const label=best?(typeof best.label==='function'?best.label():best.label):'';this.promptElement.innerHTML=best?`<kbd>E</kbd><span>${label}</span>`:'';return best}
