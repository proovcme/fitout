...
function renderDayPlan() {
  ...
  $('#dayPlanList').innerHTML=candidates.map(task=>{const blockers=hardTaskBlockers(state,task);const coverage=profileCoverage(task.skill);return `<button class="day-plan-card ${task.enabledToday?'selected':''} ${coverage?'missing-profile':''}" data-day-task="${task.id}"><span class="task-status" style="--task-color:${task.color}">${ICONS[task.reworkOf?'clean':task.id]??'↺'}</span><span><strong>${task.title}</strong><small>${task.duration} ч · ${SKILL_LABELS[task.skill]}${blockers.length?` · БЛОК: ${blockers.map(item=>item.short).join(', ')}`:task.outOfSequence?' · РИСК ПОСЛЕДОВАТЕЛЬНОСТИ':''}${coverage?coverage.kind==='missing'?` · НЕТ ПРОФИЛЬНОЙ БРИГАДЫ`:` · БРИГАДА ЧЕРЕЗ ${coverage.hours} Ч`:''}</small></span><b>P${task.priority}</b></button>`;}).join('');
