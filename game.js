...
import { COMPANY_ROLES, EMPLOYEE_UPGRADE_TREE, HQ_UPGRADE_TREE, staffTrait } from './company-content.js';
...
function renderCompanyConsole(){
  ...
  if(companyTab==='portfolio'){
    ...
    const project=state.hq?.project;const active=project?.status==='active';content.innerHTML=`<div class="company-panel"><article class="office-project-card"><div><strong>${escapeHtml(state.hq?.title??'Стол у принтера')} · уровень ${state.hq?.level??0}</strong><small>${active?`Ремонт идёт ${Math.round(project.progress*100)}%. Бюджет ${money(project.budget)}, перерасход ${money(project.overrun)}.`:'Площадь штаба открывает ступени дерева. Оснащение даёт постоянные бонусы и физически появляется в комнате.'}</small><div class="office-project-progress"><i style="--progress:${active?Math.round(project.progress*100):0}%"></i></div></div><button class="company-action" data-start-hq-project ${active?'disabled':''}>${active?'Проект идёт':'Расширить помещение'}</button></article><section class="development-panel hq-development"><header><span><small>ДЕРЕВО ШТАБА · ${state.hq.upgrades.length}/${HQ_UPGRADE_TREE.length}</small><strong>Оснащение компании</strong><em>Купленные узлы работают во всех проектах.</em></span></header>${renderUpgradeTree(HQ_UPGRADE_TREE,state.hq.upgrades,{kind:'hq',hqLevel:state.hq.level})}</section></div>`;
  }
}
