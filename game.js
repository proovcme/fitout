...
function renderTutorial() {
  ...
  let step;
  if((state.contract?.cardsPlayed?.length??0)<2)step={n:1,title:'Сначала определите цель',mentor:'«Занятость всех людей не является целью. Цель — сдать офис, получить деньги и сохранить способность строить следующий.»',text:'Сыграйте две карты договора. Вы сознательно выбираете, чем защищать поток проекта: деньгами, сроком или качеством.',lesson:'Локальная экономия бесполезна, если объект в целом не сдаётся.',target:'#contractDeck'};
  else if(state.phase==='negotiation')step={n:2,title:'Отделите договор от денег',mentor:'«Большая сумма в договоре ещё не лежит у вас в кассе. Это только обещание с приложениями.»',text:'Подпишите контракт. Сейчас поступит лишь аванс; остальная выручка появится после выполнения и приёмки этапов.',lesson:'Считайте денежный поток, а не красоту договорной цены.',target:'#startMission'};
  else if(state.phase==='preparation'&&!pm)step={n:3,title:'Найдите точку управления',mentor:'«Подрядчики умеют работать. Но никто из них не отвечает за весь объект — значит, ограничением легко станет управленческое внимание.»',text:'Наймите руководителя проекта. Он связывает отдельные бригады в одну производственную систему.',lesson:'У системы должен быть человек, который видит общий результат.',target:'[data-team-hire="pm"]'};
  else if(state.phase==='preparation'&&!movers)step={n:4,title:'Дайте потоку первое звено',mentor:'«Не набирайте всех подряд. Сначала обеспечьте работу, без которой следующий фронт вообще не появится.»',text:'Наймите перестановщиков: пока зона забита мебелью, последующие специалисты физически не смогут начать.',lesson:'Ограничение — это то, чего системе сейчас не хватает для движения.',target:'[data-map-hire="movers"]'};
  else if(state.phase==='preparation')step={n:5,title:'Не оптимизируйте всё сразу',mentor:'«Свободный специалист выглядит как потеря денег. Толпа без фронта является потерей денег на самом деле.»',text:'Подтвердите небольшой управляемый состав и выходите на объект. Остальных наймём, когда для них появится работа.',lesson:'Лишние ресурсы перед ограничением создают очередь, а не скорость.',target:'#enterSite'};
  else if(state.phase==='schedule'&&!state.masterScheduleAccepted)step={n:6,title:'Прочитайте цепочку, а не таблицу',mentor:'«График — это не список желаний. Это обещание, в каком порядке появится фронт для следующей работы.»',text:'Проверьте зависимости и примите общий график. Если сначала закрыть стены, а потом вспомнить о трубах, поток пойдёт назад.',lesson:'Технологическая последовательность важнее процента занятости.',target:'#acceptSchedule'};
  else if(state.needsPlanning&&!enabled)step={n:7,title:'Найдите ограничение сегодняшнего дня',mentor:'«Какую единственную задержку сейчас почувствует весь объект? Не выбирайте работу только потому, что её удобно начать.»',text:'Включите обследование и освобождение зоны. Они создают фронт для большинства следующих операций.',lesson:'Сначала идентифицируйте ограничение потока.',target:'#dayPlanList'};
  else if(state.needsPlanning)step={n:8,title:'Подчините план ограничению',mentor:'«Пусть остальные делают только то, что помогает узкому месту работать без остановки. Иначе они производят незавершёнку.»',text:'Запустите смену. Выбранные задачи получают людей и внимание; всё остальное сознательно ждёт.',lesson:'Не ограничение задаёт темп — остальные работы подчиняются ему.',target:'#startDay'};
  else if(!tutorial.observedBuild)step={n:9,title:'Посмотрите на поток физически',mentor:'«Не смотрите только на проценты. Где образовалась очередь? Кто ждёт фронт? Кто работает без результата для сдачи?»',text:'Понаблюдайте за площадкой: люди несут мебель, освобождают зону и создают доступ к следующей работе.',lesson:'Очередь перед работой часто показывает настоящее ограничение.',target:'.site-stage'};
  else if(!tutorial.chatSent){const chatOpen=refs.communication.classList.contains('visible')&&$('#communicationWindow').classList.contains('is-whatsapp');step={n:10,title:'Используйте ограничение лучше',mentor:'«Сначала уберите простои узкого места. Только потом покупайте ему дополнительные руки.»',text:chatOpen?'Выберите освобождение зоны или текущую ключевую работу и отправьте «сделать срочно».':'Откройте чат стройки. Время остановится, пока вы выбираете управленческое воздействие.',lesson:'Сначала используйте ограничение без потерь, затем усиливайте его.',target:chatOpen?'[data-send-urgent]':'#siteWhatsappButton'};}
  else step={n:11,title:'Ограничение переместится',mentor:'«Когда эта работа закончится, не продолжайте управлять вчерашней проблемой. Найдите новую.»',text:'Доведите первую работу до завершения и предъявления. После неё откроется следующий фронт — и система получит новое ограничение.',lesson:'Улучшение — цикл: найти, использовать, подчинить, усилить и искать заново.',target:'#taskList'};
  $('#tutorialProgress').textContent=`ГЛАВА ${step.n} ИЗ 11`;$('#tutorialTitle').textContent=step.title;$('#tutorialText').textContent=step.text;$('#tutorialMentor').textContent=step.mentor;$('#tutorialLesson').textContent=step.lesson;
