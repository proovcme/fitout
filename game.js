...
function renderTutorial() {
  ...
  let step;
  if((state.contract?.cardsPlayed?.length??0)<2)step={n:1,title:'Цоколь принимает капитана',mentor:'«PS5 была единственным ликвидным активом. Теперь у нас вместо неё принтер. Он печатает с полосой, зато это уже производственный фонд.»',text:'Вы устали быть строителем и решили стать КОРОЛЁМ ГЕНПОДРЯДА. Штаб сняли в цоколе БЦ «Банкрот». Сыграйте две карты договора и выберите, чем рисковать в первом походе: деньгами, сроком или качеством.',lesson:'Генподряд начинается не с каски, а с обещания, за которое придётся отвечать.',target:'#contractDeck'};
  else if(state.phase==='negotiation')step={n:2,title:'Первый договор ещё не победа',mentor:'«Сумма договора большая только на бумаге. В кассе сейчас сквозняк, два степлера и вера в аванс.»',text:'Подпишите контракт. Заказчик перечислит только аванс; остальные деньги придут после выполнения и приёмки работ. Если работы не приняли, богатство остаётся литературным жанром.',lesson:'Договор показывает масштаб надежды. Касса показывает положение дел.',target:'#startMission'};
  else if(state.phase==='preparation'&&!pm)step={n:3,title:'Соберите тех, кому тоже некуда отступать',mentor:'«Хорошая команда — не толпа героев. Это люди, которые знают, чей сегодня пожар и кто подписывает акт.»',text:'Наймите руководителя проекта. В цоколе уже собрались такие же будущие легенды, но без человека, отвечающего за весь результат, они образуют кружок по обмену срочными сообщениями.',lesson:'У объекта должен быть один ответственный за целое, а не семь ответственных за объяснение.',target:'[data-team-hire="pm"]'};
  else if(state.phase==='preparation'&&!movers)step={n:4,title:'Найдите первое узкое место',mentor:'«Пока мебель стоит в проходе, маляр может быть хоть волшебником. Красить ему всё равно негде.»',text:'Наймите перестановщиков. Они освободят рабочую зону и откроют фронт следующим бригадам. Это первый настоящий выбор генподрядчика: оплачивать надо не всех полезных людей, а тех, без кого сейчас не движется объект.',lesson:'Сначала устраните то, что держит весь поток.',target:'[data-map-hire="movers"]'};
  else if(state.phase==='preparation')step={n:5,title:'Не набирайте армию в прихожую',mentor:'«Десять подрядчиков без фронта строят только очередь в курилку. Причём удивительно качественно.»',text:'Подтвердите небольшой состав и выходите на объект. Остальные бригады можно привлечь позже, когда для них появится работа и деньги перестанут притворяться бесконечными.',lesson:'Лишние люди создают толпу, но не обязательно скорость.',target:'#enterSite'};
  else if(state.phase==='schedule'&&!state.masterScheduleAccepted)step={n:6,title:'Нарисуйте курс, пока берег виден',mentor:'«График — это маршрут обещаний. Перепутаете порядок — сначала построите стену, потом героически сломаете её ради кабеля.»',text:'Проверьте зависимости и примите общий график. Каждая завершённая работа должна создавать фронт для следующей, а не материал для будущей переделки.',lesson:'Правильная последовательность экономит больше, чем самый громкий прораб.',target:'#acceptSchedule'};
  else if(state.needsPlanning&&!enabled)step={n:7,title:'Первое утро империи',mentor:'«Король генподряда отличается от обычного человека тем, что до кофе уже знает, какая работа сорвёт ему вечер.»',text:'Включите обследование и освобождение зоны в план дня. Эти работы открывают путь большинству последующих операций. Не пытайтесь запустить всё сразу: объект не обязан разделять ваш энтузиазм.',lesson:'План дня начинается с работы, которая открывает остальные.',target:'#dayPlanList'};
  else if(state.needsPlanning)step={n:8,title:'Команда получает курс',mentor:'«Теперь запустите смену. Если всё приоритетное, значит, приоритета у вас нет — только нервный шрифт.»',text:'Начните день. Выбранные задачи получат людей и внимание, остальные сознательно подождут своего фронта. Это не бездействие, а редкий случай управляемого бездействия.',lesson:'Все ресурсы должны помогать главной работе дня, а не изображать занятость.',target:'#startDay'};
  else if(!tutorial.observedBuild)step={n:9,title:'Стройка начинает двигаться',mentor:'«Не верьте одним процентам. Смотрите, что исчезает, что появляется и перед какой работой собралась очередь из людей с очень деловыми лицами.»',text:'Понаблюдайте за площадкой. Мебель уносят, зона освобождается, люди работают или честно бездельничают. Физическое изменение объекта показывает правду быстрее отчёта.',lesson:'Настоящее состояние проекта видно на площадке, а не в цвете ячейки.',target:'.site-stage'};
  else if(!tutorial.chatSent){const chatOpen=refs.communication.classList.contains('visible')&&$('#communicationWindow').classList.contains('is-whatsapp');step={n:10,title:'Первое грозное сообщение',mentor:'«Иногда ограничению не нужны новые люди. Ему нужно, чтобы старые перестали обсуждать, почему задача поставлена некорректно.»',text:chatOpen?'Выберите освобождение зоны или текущую ключевую работу и отправьте сообщение «сделать срочно». Оно может ускорить бригаду, а может пополнить их коллекцию скриншотов о заказчиках.':'Откройте чат стройки. Пока вы выбираете управленческое воздействие, игровое время остановится.',lesson:'Сначала уберите простой имеющихся ресурсов, потом покупайте новые.',target:chatOpen?'[data-send-urgent]':'#siteWhatsappButton'};}
  else step={n:11,title:'Первая работа поднимает флаг',mentor:'«Доведите её до конца и предъявите. Непринятая работа — это не результат, а дорогое мнение подрядчика о самом себе.»',text:'Завершите первую работу и предъявите её заказчику. После приёмки придут этапные деньги, откроется следующий фронт и прежнее узкое место перестанет быть главным. До короны ещё далеко, но впереди уже целое море чужих офисов.',lesson:'Завершили, приняли, получили деньги — затем ищите новое ограничение.',target:'#taskList'};
  $('#tutorialProgress').textContent=`ГЛАВА 1 · ${step.n}/11`;$('#tutorialTitle').textContent=step.title;$('#tutorialText').textContent=step.text;$('#tutorialMentor').textContent=step.mentor;$('#tutorialLesson').textContent=step.lesson;
