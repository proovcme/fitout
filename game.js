...
function renderTutorial() {
  ...
  else if(state.phase==='preparation'&&!movers)step={n:4,title:'Найдите первое узкое место',mentor:'«Пока мебель стоит в проходе, маляр может быть хоть волшебником. Красить ему всё равно негде. Максимум — тумбочку.»',text:'Наймите перестановщиков. Они освободят рабочую зону и откроют фронт следующим бригадам. Это первый настоящий выбор генподрядчика: оплачивать надо не всех полезных людей, а тех, без кого сейчас не движется весь поток. Но запомните — перестановщики умеют переставлять, а не проектировать, сверлить и красить.',lesson:'Сначала устраните то, что держит весь поток.',target:'[data-map-hire="movers"]'};
  else if(state.phase==='preparation'&&!designers)step={n:5,title:'Первый фронт — ещё не весь объект',mentor:'«Вы наняли передвижников. Прекрасно: теперь шкаф переедет профессионально. Но рабочий проект он по дороге не выпустит.»',text:'Наймите проектировщиков. У каждой работы есть профиль исполнителя: перестановка, проектирование, демонтаж, инженерия, отделка и другие. Одна бригада не закрывает весь график. Сейчас мы берём второй необходимый профиль; позже карточки работ сами покажут, кого не хватает, и дадут открыть подходящих подрядчиков.',lesson:'Перед началом работы проверьте не только фронт, но и наличие профильного исполнителя.',target:'[data-map-hire="designers"]'};
  else if(state.phase==='preparation')step={n:6,title:'Не набирайте армию в прихожую',mentor:'«Десять подрядчиков без фронта строят только очередь в курилку. Причём удивительно качественно. Но один перестановщик на весь объект — тоже не стратегия.»',text:state.preparationConfirmed?'Состав зафиксирован. Это не значит, что нанятые люди умеют всё: на объекте карточки будущих работ продолжат показывать отсутствующие профили. Выходите на площадку — дальше донабор будет доступен через кнопку «Найти» прямо в предупреждении.':'Посмотрите предупреждение под кнопкой подтверждения: там перечислены профили графика, для которых пока нет отдельной бригады. Можно нанять всех заранее, а можно донабирать по мере открытия фронтов — универсальная хозбригада подстрахует, но медленнее и с риском качества. Подтвердите осознанный состав.',lesson:'Нужных людей должно быть достаточно для ближайшего фронта, но не больше, чем выдержат касса и площадка.',target:state.preparationConfirmed?'#enterSite':'#confirmPreparationButton'};
