...
function renderPreparation() {
  ...
  $('#confirmPreparationButton').textContent=state.preparationConfirmed?'Состав подтверждён ✓':'Подтвердить выбранный состав';$('#confirmPreparationButton').disabled=Boolean(state.preparationConfirmed);$('#enterSite').disabled=!state.preparationConfirmed;$('#prepConfirmStatus').textContent=state.preparationConfirmed?`${uncoveredText} Можно выходить на объект.`:`${uncoveredText} Состав можно менять и отзывать без штрафа.`;
}
...
function renderTutorial() {
  ...
  else if(state.phase==='preparation')step={n:6,title:'Не набирайте армию в прихожую',mentor:'«Десять подрядчиков без фронта строят только очередь в курилку. Причём удивительно качественно. Но одна подсобная бригада на весь объект — тоже не производственная система.»',text:state.preparationConfirmed?'Состав зафиксирован. Это не значит, что нанятые люди умеют всё: на объекте карточки будущих работ продолжат показывать отсутствующие профили. Выходите на площадку — дальше донабор будет доступен через кнопку «Найти» прямо в предупреждении.':'Посмотрите предупреждение под кнопкой подтверждения: там перечислены профили графика, для которых пока нет отдельной бригады. Можно нанять всех заранее, а можно донабирать по мере открытия фронтов — штатная хозбригада подстрахует, но медленнее и с риском качества. Подтвердите осознанный состав.',lesson:'Нужных людей должно быть достаточно для ближайшего фронта, но не больше, чем выдержат касса и площадка.',target:state.preparationConfirmed?'#enterSite':'#confirmPreparationButton'};
