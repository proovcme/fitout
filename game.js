...
function renderMainMenu() {
  ...
  const newPlayer=!liveProjects.length&&(organization.projectsCompleted??0)===0;newGameButton.querySelector('b').textContent='Новая игра';newGameButton.querySelector('small').textContent=newPlayer?'Учебный объект с Семёном Ильичом':'Начать с нового портфеля';
...
$('#newGameButton').addEventListener('click',()=>{const live=state.portfolio?.projects?.some(project=>!project.summary?.completed);if(live){companyTab='market';renderMainMenu();showToast('Текущие объекты сохранены. Можно взять ещё один — максимум три одновременно.','done');return;}resetGame();});
