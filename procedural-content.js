...
const SUBJECTS = {
  ...
  general:['коробка «НЕ ТЕРЯТЬ»','единственный грузовой лифт','шкаф, который не проходит','сорок третий стул','наклейки на мебель','тележка без колеса','чужой сервер','диван из Италии'],
...
const AMBIENT_BEATS = {
  management: [
    { kind:'briefing', text:'Прораб собрал короткую планёрку. Она уже длиннее короткой.' },
    { kind:'inspection', text:'Техзаказчик идёт по объекту и фотографирует то, чего ещё нет в отчёте.' },
    { kind:'measurement', text:'Снова меряют проход. В проекте он почему-то шире.' },
  ],
  general: [
    { kind:'delivery', text:'Машина встала во дворе. Водитель уверяет, что разгрузка была «до борта».' },
    { kind:'carry', text:'Подсобники несут шкаф по маршруту, которого нет на плане.' },
    { kind:'argument', text:'Грузовой лифт поделили между людьми, мебелью и чьим-то срочным диваном.' },
  ],
  paint: [
    { kind:'sample', text:'Маляры делают ещё один выкрас того же самого, но окончательного серого.' },
    { kind:'argument', text:'Архитектор и маляр сравнивают стену с фотографией стены в телефоне.' },
    { kind:'cleanup', text:'Перед покраской подняли пыль, чтобы потом было что убрать.' },
  ],
  electric: [
    { kind:'drill', text:'Где-то сверлят. Судя по лицам, не там, где ожидали.' },
    { kind:'power-test', text:'Электрики включили свет по секциям. Одна секция включила соседнюю.' },
    { kind:'measurement', text:'Трассу кабеля снова размечают после встречи кабеля с воздуховодом.' },
  ],
  furniture: [
    { kind:'assembly', text:'Сборщики нашли лишний винт. Мебель считает его обязательным.' },
    { kind:'carry', text:'Стол несут через готовый проход с точностью до миллиметра и штукатурки.' },
    { kind:'argument', text:'Левую правую опору обсуждают уже на уровне высшего руководства.' },
  ],
  cleaning: [
    { kind:'cleanup', text:'Клининг открыл настоящий цвет пола и сразу попросил никого не ходить.' },
    { kind:'inspection', text:'Уборку принимают фонариком. Стройка делает вид, что так и было задумано.' },
    { kind:'carry', text:'Мешки уходят на склад временного вечного хранения.' },
  ],
  design: [
    { kind:'measurement', text:'Архитектор сверяет реальный угол с концептуально прямым.' },
    { kind:'sample', text:'На стене появился образец материала, который уже сняли с производства.' },
    { kind:'briefing', text:'Проектировщики защищают решение, пока строители защищают сроки.' },
  ],
  documentation: [
    { kind:'inspection', text:'Специалист ИД ищет подпись человека, который сегодня на другом объекте.' },
    { kind:'briefing', text:'Исполнительные схемы обсуждают вокруг принтера, печатающего предыдущую версию.' },
    { kind:'measurement', text:'Размер в акте проверяют рулеткой. Рулетка предлагает третий вариант.' },
  ],
  support: [
    { kind:'briefing', text:'Команда синхронизировалась. Подрядчики синхронно посмотрели на часы.' },
    { kind:'inspection', text:'Технадзор молча фотографирует узел. Это звучит громче замечания.' },
    { kind:'measurement', text:'Комплектатор считает коробки и получает новое число при каждом пересчёте.' },
  ],
};

export function generateAmbientBeat(skill = 'management', token = 0) {
  const beats=AMBIENT_BEATS[skill]??AMBIENT_BEATS.management;
  const beat=pick(beats,mix(token+skill.length,83));
  return {...beat,skill};
}
...
export function bubbleFor(role, token = 0) {
  ...
  const casual = ['###@!#!!','НЕ ПО ПРОЕКТУ!','КТО ЗАКАЗАЛ?!','ГДЕ РУЛЕТКА?','ЭТО НЕ МОЁ','СРОЧНО?!','А ДОПЫ?','КТО СЛОМАЛ?','НЕ ТРОГАЙ','УЖЕ КРАСИЛИ','ЧЕЙ КАБЕЛЬ?','ГДЕ ПРОРАБ?'];
