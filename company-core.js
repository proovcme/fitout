...
function defaultCompany(legacy={}){
  return {
    ...
    crisis:legacy.crisis??null,bankrupt:Boolean(legacy.bankrupt),reserveTarget:legacy.reserveTarget??.1,lastPayrollPeriod:legacy.lastPayrollPeriod??null,contractorClaims:clone(legacy.contractorClaims??[]),
  };
}
...
export function advanceCompanyDay(state,{simulateBackground=true}={}){
  ...
  for(const claim of state.company.contractorClaims??[]){
    if(claim.status==='paid')continue;
    const age=Math.max(0,state.companyCalendar.day-(claim.openedDay??state.companyCalendar.day));
    const relation=state.contractorNetwork?.find(item=>item.id===claim.contractorId);if(relation)relation.relationship=clamp((relation.relationship??50)-2,0,100);
    if(age>=3&&!claim.noticeSent){claim.noticeSent=true;state.companyInbox.unshift({id:`claim-notice-${claim.id}`,kind:'contractor',title:`${claim.company}: досудебная претензия`,text:`Остаток ${claim.balance}К не оплачен. Подрядчик перестал ставить смайлик после слова «коллеги».`,urgent:true,createdDay:state.companyCalendar.day});}
    if(age>=7&&!claim.lawsuitFiled){claim.lawsuitFiled=true;claim.status='lawsuit';claim.balance=Math.ceil(claim.balance*1.35);state.company.reputation=clamp(state.company.reputation-6,0,100);state.companyInbox.unshift({id:`claim-lawsuit-${claim.id}`,kind:'contractor',title:`${claim.company} подал в суд`,text:`Долг вырос до ${claim.balance}К с расходами и неустойкой. Теперь «порешать» надо ещё и с госпошлиной.`,urgent:true,createdDay:state.companyCalendar.day});}
  }
